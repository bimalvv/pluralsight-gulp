...
var port = process.env.PORT || config.defaulPort;
...
gulp.task('inject', ['wiredep', 'styles', 'templatecache'], function() {
	log('Wire up the app css into the html and calls wiredep.');
...
gulp.task('optimize', ['inject', 'fonts', 'images'], function() {
	...
	return gulp.src(config.index)
		...
		/* Renames app.js to app-123sbv.js */
		.pipe($.rev())
		/* Brings back the previously
		   filtered out HTML files. */
		.pipe(assets.restore())
		.pipe($.useref())
		/* Rewrite occurences of filenames which
		   have been renamed by gulp-rev */
		.pipe($.revReplace())
...
function startBrowserSync(isDev) {
	...
	log('Starting btowser-sync on port ' + port);
